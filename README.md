...
- 对话式图纸 / 报告工作流：在“智能对话”中上传设计报告 `.pdf` 或平面图 `.dxf/.dwg` 后，系统保存原始资料；CAD 仅解析为二维平面图证据，不执行三维建模。
- 照度与方案交付：基于已确认的任务书、证据和灯具候选执行流明法初算、规则检查，并生成 DIALux 交接包和设计报告。正式照度、UGR 和规范符合性仍须在 DIALux evo 或等效专业软件中复核。
...
注：Blender MCP 三维建模及其附属工作流已移除；CAD 文件仅支持二维平面图解析。
