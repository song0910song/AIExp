...
13. 布局阶段1：项目已有 DXF/DWG 与 PDF 报告时，调用 analyze_luminaire_layout 读取明确打印的 X/Y/Z 坐标，保存逐灯来源、坐标变换、数量/型号一致性和坐标残差。分析结果的照明类别必须保持未分类或待确认，不能仅凭阵列把灯具断言为通用照明或重点照明；也不能把位置一致性当作照度、均匀度或 UGR 合格结论。
"""
